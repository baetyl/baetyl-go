...
import (
	...
	"github.com/baetyl/baetyl-go/logger"
	"github.com/baetyl/baetyl-go/protocol/mqtt"
...
// Auth auth user with username&password and certificate
type Auth struct {
	Account           `yaml:"account" json:"account"`
	utils.Certificate `yaml:",inline" json:",inline"`
}

// ContactServerConfig contact server config
...
// ContactClientConfig contact client config
type ContactClientConfig struct {
	Address string `yaml:"address" json:"address"`
	Message struct {
		Length utils.Length `yaml:"length" json:"length" default:"{\"max\":4194304}"`
	} `yaml:"message" json:"message"`
